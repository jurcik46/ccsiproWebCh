...
namespace WebChromiumCcsipro.Controls.Services
{
    public class SignatureService : ISignatureService
    {
        ...
        public void StartSign()
        {
            ...
            {
                InProcces = true;
                Messenger.Default.Send<ChangeIconMessage>(new ChangeIconMessage() { Icon = TrayIconsStatus.Working });
                _appRomaingPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "CCSIPRO", "bozp");

                Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationSearchingDocument, IconType = Notifications.Wpf.NotificationType.Information, ExpTime = 300 });
                SignatureFileModel = ApiService.GetDocumentToSignature();
                if (SignatureFileModel != null)
                {
                    Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationSavingDocument, IconType = Notifications.Wpf.NotificationType.Information, ExpTime = 300 });
                    // prehodi lomku 
                    ...
                    if (CreateDirectory(ref directhoryPath))
                    {
                        ...
                        if (SaveFile(directhoryPath, fileName, SignatureFileModel.File))
                        {
                            ...
                            Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationStarginSingSoftware, IconType = Notifications.Wpf.NotificationType.Information, ExpTime = 300 });
                            ...
                            if (SignFile(processName, SettingsService.ProgramPath, filePath))
                            {
                                ...
                                Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationUploadingDocument, IconType = Notifications.Wpf.NotificationType.Information, ExpTime = 300 });
                                ...
                                if (ApiService.UploadSignedDocument(SignatureFileModel.Hash, SignatureFileModel.PdfFilePath.Substring(1, SignatureFileModel.PdfFilePath.Length - 1), filePath))
                                {
                                    ...
                                    Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationSuccessUploadedDocument, IconType = Notifications.Wpf.NotificationType.Success, ExpTime = 10 });
                                }
                                else
                                {
                                    //                                    Messenger.Default.Send<BozpStatusPusherMessage>(new BozpStatusPusherMessage() { Status = "500" });
                                    Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationFialedUploadedDocument, IconType = Notifications.Wpf.NotificationType.Error });
                                }
                            }
                            else
                            {
                                ...
                                Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationClosedDocument, IconType = Notifications.Wpf.NotificationType.Warning });
                            }
                        }
                        else
                        {
                            Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationFailedSavingDocument, IconType = Notifications.Wpf.NotificationType.Error });
                        }
                    }
                    else
                    {
                ...
                {
                    Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationNotFoundDocument, IconType = Notifications.Wpf.NotificationType.Warning });
                    //                    Messenger.Default.Send<BozpStatusPusherMessage>(new BozpStatusPusherMessage() { Status = "404" });
                }

                Messenger.Default.Send<ChangeIconMessage>(new ChangeIconMessage() { Icon = TrayIconsStatus.Online });
                InProcces = false;
            }
        }
            ...
            using (var process = Process.Start(startInfo))
            {
                ...
                do
                {
                    ...
                    if (!foundWindow && found)
                    {
                        ...
                        foundWindow = true;
                        Messenger.Default.Send<NotifiMessage>(new NotifiMessage() { Title = lang.SignatureServiceNotificationTitle, Msg = lang.SignatureServiceNotificationSuccessOpenDocument, IconType = Notifications.Wpf.NotificationType.Information, ExpTime = 300 });
                    }
